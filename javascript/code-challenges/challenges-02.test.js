...
const raisedToTheThird = (arr) => {
  ...
  let newaArr = [];
  arr.forEach(val => {
    let newNumber = Math.pow(val,3);
    newaArr.push(newNumber);
  });
  return newaArr;
};
...
const charCode = (arr) => {
  ...
  let newArr = [];
  arr.map( val =>{
    let newSring = val.charCodeAt();
    newArr.push(newSring);
  });
  return newArr;
};
